(function() {
    /* Object Defeintion */
    // <unit-name> : [[synonyms],converstion_factor,addition_factor,unit_type,[numerator_unit],[denominator_unit]]
    // order of operations addition_factor then converstion_factor
  ...
  UNITS = {
   ...
   "<tera>"  :  [["T","Tera","tera"], 1e12, 0 , "prefix"],
    ...
    "<celsius>" : [["degC","celsius","celsius","centigrade"], 1.0, 0 , "temperature", ["<kelvin>"]],
    "<fahrenheit>" : [["degF","fahrenheit"], 1/1.8, 0 , "temperature", ["<kelvin>"]], 
    "<temp-K>"  : [["tempK"], 1.0, 0 , "temperature", ["<temp-K>"]],
    "<temp-C>"  : [["tempC"], 1.0, 0 , "temperature", ["<temp-K>"]],
    "<temp-F>"  : [["tempF"], 1/1.8, 0 , "temperature", ["<temp-K>"]],
    ...
    /* concentration */ //Doesn't account for volume
    ...
    "<ppi>" : [["ppi"], 1, "resolution", 0 , ["<pixel>"], ["<inch>"]],
    "<dpi>" : [["dpi"], 1, "typography", 0 , ["<dot>"], ["<inch>"]],

    /* other */
    "<cell>" : [["cells","cell"], 1, 0 , "counting", ["<each>"]],
    "<each>" : [["each"], 1.0, "counting", 0 , ["<each>"]],
    "<count>" : [["count"], 1.0, "counting", 0 , ["<each>"]],
  ...
  BASE_UNITS = ['<meter>','<kilogram>','<second>','<mole>', '<farad>', '<ampere>','<radian>','<kelvin>','<temp-K>','<byte>','<dollar>','<candela>','<each>','<steradian>'];
  UNITY = '<1>';
  UNITY_ARRAY= [UNITY];
  SCI_NUMBER = "([+-]?\\d*[.]?\\d+(?:[Ee][+-]?)?\\d*)";
  ...
  POWER_OP = "(?:\\^|\\*\\*)?"
  TOP_REGEX = new RegExp("([^ \\*]+?)" + POWER_OP + "([\\d-]+)");
  ...
  window.Qty = function Qty(init_value) {
    this.scalar = null;
  ...
  */
  ...
  window.Qty.prototype = {
    ...
    */
    ...
    units: function() {
      ...
      this.unit_name = on.join("*") + ((compareArray(output_d, UNITY_ARRAY)) ? '':'/' + od.join("*"));
      return this.unit_name;
    },
  ...
  /*function mul_safe() {
    ...
    for(var i = 0; i < arguments.length; i++) {
      var arg = arguments[i];
      var fractional = arg - Math.floor(arg);
      if(fractional !== 0) {
         decimals = (decimals || 0) - exponent_of(fractional);
      }
  ...
  UNIT_TEST_REGEX = new RegExp("^\\s*((" + PREFIX_REGEX + ")*?(" + UNIT_REGEX + ")\\s*\\*?\\s*)+$");
})();
